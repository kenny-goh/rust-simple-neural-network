...
impl DenseLayer {
    pub fn new(input_size: usize,
               num_of_nodes: usize,
               activation: Activation,
               strategy: &RandomWeightInitStrategy) -> DenseLayer {
    ...
    pub fn back_props(&mut self,
        ...
        let m = prev_activation.size(1);

        let (error,
            derivative_weights,
            derivative_biases) = self.calc_gradient(params, partial_error, z, &prev_activation, m);

        match params.optimizer {
            Optimizer::SGD => {
                self.back_props_sgd(epoch, train_size, params, derivative_weights, derivative_biases);
            }
            Optimizer::SGDMomentum => {
                self.back_props_sgd_momentum(epoch, train_size, params, derivative_weights, derivative_biases);
            }
            Optimizer::RMSProps => {
                self.back_props_rms(epoch, train_size, params, derivative_weights, derivative_biases);
            }
        }
    ...
    fn back_props_rms(&mut self, epoch: i32, train_size: f32, params: &TrainParameters, derivative_weights: Tensor2D, derivative_biases: Tensor2D) {
        ...
        self.weights = if params.l2 >= 0. {
            // regularization
            (&self.weights - &(&derivative_weights / &sdw.sqrt() * learning_rate)) * (1. - (learning_rate * params.l2 / train_size))
        } else {
    ...
    fn back_props_sgd_momentum(&mut self, epoch: i32,
                               train_size: f32,
                               params: &TrainParameters,
        ...
        self.weights = if params.l2 >= 0. {
            (&self.weights - &(&vdw * learning_rate)) * (1. - (learning_rate * params.l2 / train_size))
        } else {
    ...
    fn back_props_sgd(&mut self, epoch: i32, train_size: f32, params: &TrainParameters, derivative_weights: Tensor2D, derivative_biases: Tensor2D) {
        let learning_rate = (1. / (1. + params.learning_rate_decay * epoch as f32)) * params.learning_rate;
        self.weights = if params.l2 >= 0. {
            (&self.weights - &(learning_rate * derivative_weights)) * (1. - (learning_rate * params.l2 / train_size))
        } else {
    ...
    fn calc_gradient(&mut self,
                     ...
                     z: Tensor2D,
                     prev_activation: &&Tensor2D,
                     m: f32) -> (Tensor2D, Tensor2D, Tensor2D) {
        let grad_clipping = params.gradient_clipping.as_ref();
        return if grad_clipping.is_some() {
            let (clip_min, clip_max) = grad_clipping.unwrap();
            let error = (self.activation.compute_derivative(z) * partial_error.clip(*clip_min, *clip_max));
            let derivative_weights = ((error.dot(&prev_activation.t())) * (1.0 / m)).clip(*clip_min, *clip_max);
            let derivative_biases = (error.sum_keep_dims() * (1.0 / m)).clip(*clip_min, *clip_max);
            (error, derivative_weights, derivative_biases)
        } else {
            let error = self.activation.compute_derivative(z) * partial_error;
            let derivative_weights = error.dot(&prev_activation.t()) * (1.0 / m);
            let derivative_biases = error.sum_keep_dims() * (1.0 / m);
            (error, derivative_weights, derivative_biases)
        };
    }
